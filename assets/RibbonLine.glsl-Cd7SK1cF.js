...
return worldToScreenPerDistanceRatio / segmentDistanceToCamera;
  ...
  `),s.uniforms.add(new bi("stipplePatternTexture",n=>n.stippleTexture),new X("stipplePatternPixelSizeInv",n=>1/Lt(n))),e.stippleOffColorEnabled&&s.uniforms.add(new De("stippleOffColor",n=>xa(n.stippleOffColor))),a.include(Xe),s.code.add(c`float getStippleSDF(out bool isClamped) {
...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Lt(a){const e=a.stipplePattern;return e?_a(a.stipplePattern)/e.pixelRatio:1}const Pa=.4,Rt=64,wa=Rt/2,za=wa/5,Oa=Rt/za,ws=.25;function Ca(a,e){const t=a.vertex,i=e.hasScreenSizePerspective;et(t),t.uniforms.get("markerScale")==null&&t.constants.add("markerScale","float",1),t.constants.add("markerSizePerLineWidth","float",Oa).code.add(c`
...
vTransitionType = timeStamps.w;`);const{animation:o}=e;o===3&&n.constants.add("decayRate","float",2.3),n.code.add(c`
    float getTrailOpacity(float x) {
      ${Aa(o)}
    }`),n.uniforms.add(new X("timeElapsed",l=>l.timeElapsed),new X("trailLength",l=>l.trailLength),new X("speed",l=>l.animationSpeed),new De("timingOptions",l=>ve(Ea,l.startTime,l.endTime,l.fadeInTime,l.fadeOutTime))),n.code.add(c`float fadeIn(float x) {
...
return animatedColor;
        ...
        return (exp(-decayRate * x) - cutOff) / (1.0 - cutOff);`;default:return"return 1.0;"}}const Ea=Ze(),Se=1;function Wt(a){const e=new la,{attributes:t,varyings:i,vertex:s,fragment:n}=e,{applyMarkerOffset:o,draped:l,output:r,capType:d,stippleEnabled:p,falloffEnabled:v,roundJoins:g,wireframe:u,innerColorEnabled:f,hasAnimation:y,hasScreenSizePerspective:b}=a;n.include(yi),e.include(Sa,a),e.include($a,a),e.include(_i,a),e.include(xi,a),e.include(Da,a);const U=o&&!l;U&&(s.uniforms.add(new X("markerScale",m=>m.markerScale)),e.include(Ca,{space:2,hasScreenSizePerspective:b})),Ti(s,a),s.uniforms.add(new $i("inverseProjectionMatrix",m=>m.camera.inverseProjectionMatrix),new Pi("nearFar",m=>m.camera.nearFar),new X("miterLimit",m=>m.join!=="miter"?0:m.miterLimit),new wi("viewport",m=>m.camera.fullViewport)),s.constants.add("LARGE_HALF_FLOAT","float",65500),t.add("position","vec3"),t.add("previousDelta","vec4"),t.add("nextDelta","vec4"),t.add("lineParameters","vec2"),t.add("u0","float"),i.add("vColor","vec4"),i.add("vpos","vec3",{invariant:!0}),i.add("vLineDistance","float"),i.add("vLineWidth","float");const q=p;q&&i.add("vLineSizeInv","float");const h=d===2,A=p&&h,E=v||A;E&&i.add("vLineDistanceNorm","float"),h&&(i.add("vSegmentSDF","float"),i.add("vReverseSegmentSDF","float")),s.code.add(c`vec2 perpendicular(vec2 v) {
return vec2(v.y, -v.x);
}
...
float segmentLengthPseudoScreen = segmentLengthRender * discreteWorldToScreenRatio;`),s.uniforms.add(new X("stipplePatternPixelSize",m=>Lt(m))),s.main.add(c`float patternLength = patternLineSize * stipplePatternPixelSize;
